...
// Test full cluster handling by pointing the cluster handler to an *nonempty* mock fake client,
// producing some events and then comparing state. In addition to the above test case, this makes
// sure that existing objects are detected correctly at startup
func TestClusterState(t *testing.T) {
	var objs []runtime.Object
	// Append two ingress pods
	for i := 0; i < 2; i++ {
		objs = append(objs, &v1coreapi.Pod{
			ObjectMeta: metav1.ObjectMeta{
				Name:      "ingress-nginx-" + strconv.Itoa(i),
				Namespace: "ingress-nginx",
				Labels: map[string]string{
	...
	// Append one ingress
	objs = append(objs, &v1beta1extensionsapi.Ingress{
		ObjectMeta: metav1.ObjectMeta{
			Name:      "dummy-ingress",
			Namespace: "ingress-nginx",
		},
	...
	g := gomega.NewGomegaWithT(t)
	client, uut := createFakeClientsetAndUUT(t, objs...)

	// Create third pod
	_, err := client.CoreV1().Pods("ingress-nginx").Create(&v1coreapi.Pod{
		ObjectMeta: metav1.ObjectMeta{
			Name:      "ingress-nginx-eventpod",
			Namespace: "ingress-nginx",
			Labels: map[string]string{
				"app.kubernetes.io/name": "ingress-nginx",
			},
